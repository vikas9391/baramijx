...
function makeAdminToken() {
  return crypto.createHmac('sha256', SESSION_SECRET || '').update(String(ADMIN_USERNAME || '')).digest('hex');
}
...
async function initVisitorTable() {
  if (!pool) {
    console.warn('DATABASE_URL is not configured; visitor counter is disabled.');
    return;
  }
  await pool.query(`
...
app.post('/api/visitor/visit', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Unable to update visitor counter.' });
  }
});
...
app.get('/api/visitor/count', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Unable to load visitor counter.' });
  }
});
...
app.get('/api/admin/visitors', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('Admin visitor counter error:', err);
    res.status(500).json({ error: 'Unable to load visitor counter.' });
  }
});
...
app.patch('/api/admin/visitors', requireAdmin, async (req, res) => {
  try {
    ...
    if (!Number.isSafeInteger(desired) || desired < 0) {
      return res.status(400).json({ error: 'Displayed visitor count must be a non-negative whole number.' });
    }
    const result = await pool.query(`
      UPDATE visitor_counter
      SET display_offset = $1 - actual_visits, updated_at = NOW()
      WHERE id = 1
      RETURNING actual_visits, display_offset, actual_visits + display_offset AS displayed_visits, updated_at
    `, [desired]);
    res.json(result.rows[0]);
  } catch (err) {
    console.error('Admin visitor counter update error:', err);
    res.status(500).json({ error: 'Unable to update visitor counter.' });
  }
});
...
start().catch((err) => {
  console.error('Visitor gateway startup failed:', err);
  process.exit(1);
});
