...
export default function Header({ language, setLanguage, onOpenProgram }: HeaderProps) {
  ...
  useEffect(() => {
    try {
      setShowProgramHint(localStorage.getItem('pml-program-hint-seen') !== '1');
    } catch {
      setShowProgramHint(true);
    }
  }, []);
  ...
  const hintText = {
    ar: 'اضغط هنا لعرض البرنامج الانتخابي',
    fr: 'Cliquez ici pour voir le programme',
  ...
  const openProgram = () => {
    closeMobile();
    setShowProgramHint(false);
    try {
      localStorage.setItem('pml-program-hint-seen', '1');
    } catch {
            ...
            <div className="relative shrink-0">
              ...
              >
                ...
                <div className="min-w-0 max-w-[180px] sm:max-w-none">
                  <div className="text-[13px] sm:text-lg font-bold leading-tight truncate group-hover:text-accent transition-colors" style={{ color: '#0F1419' }}>الحزب المغربي الحر</div>
