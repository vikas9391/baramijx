...
function statusLabel(status: Problem['status']) { return status === 'in_progress' ? 'In progress' : status === 'resolved' ? 'Resolved' : 'New'; }

function Icon({ name }: { name: 'grid' | 'chat' | 'flag' | 'refresh' | 'logout' | 'menu' | 'close' | 'users' | 'message' | 'alert' | 'check' }) {
...
export default function AdminPage() {
  ...
  async function loadDashboard() {
    ...
    try {
      const [statsData, conversationsData, problemsData] = await Promise.all([
        api('/api/admin/stats'), api('/api/admin/conversations'), api('/api/admin/problems'),
      ]);
    ...
    try {
      ...
      setVisitorStats(visitorData); setVisitorInput(String(visitorData.displayed_visits));
    } catch { /* Visitor API must never prevent the main dashboard from rendering. */ }
  }
  ...
  async function saveVisitorCount(event: React.FormEvent) {
  ...
  if (checking) return <div className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-600">Loading admin panel...</div>;
  if (!authenticated) return <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4"><form onSubmit={handleLogin} className="w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-xl p-7"><div className="text-center mb-7"><div className="mx-auto mb-4 h-14 w-14 rounded-2xl bg-primary text-primary-foreground flex items-center justify-center font-bold text-xl">P</div><h1 className="text-3xl font-bold">Admin Portal</h1><p className="text-sm text-slate-500 mt-2">Private campaign administration</p></div><label className="block mb-4"><span className="text-sm font-medium">Username</span><input value={username} onChange={e => setUsername(e.target.value)} className="mt-1.5 w-full px-4 py-3 rounded-xl border bg-slate-50" autoComplete="username" /></label><label className="block mb-4"><span className="text-sm font-medium">Password</span><input value={password} onChange={e => setPassword(e.target.value)} type="password" className="mt-1.5 w-full px-4 py-3 rounded-xl border bg-slate-50" autoComplete="current-password" /></label>{loginError && <div className="mb-4 rounded-xl bg-red-50 border border-red-100 p-3 text-sm text-red-700">{loginError}</div>}<button className="cta-button w-full !rounded-xl !py-3.5" type="submit">Sign in to dashboard</button></form></div>;
